<script>
    ...
    function viewTranscript(recording) {
        const mdPath =
            transcriptPath[recording.filename] ||
            recording.path.replace(/\.(wav|m4a)$/i, ".md");
        viewingTranscript = {
            path: mdPath,
    ...
    async function checkForTranscripts() {
        ...
        for (const recording of $recordings) {
            try {
                ...
                console.error(
                    "Failed to check transcript for",
                    recording.filename,
                    error,
                );
            }
    ...
    $: console.log(
        "[RecordingsList] selectedRecording changed:",
        selectedRecording,
    );
</script>
            ...
            <div class="recordings-list">
                {#each $recordings as recording (recording.path)}
                    <div
                        ...
                        on:keydown={(e) =>
                            e.key === "Enter" && showRecordingDetail(recording)}
                        role="button"
                        tabindex="0"
                    >
                                ...
                                <div class="recording-info">
                                    ...
                                    {:else}
                                        ...
                                        <div class="recording-meta">
                                            <span
                                                class="format-badge {recording.format}"
                                                >{recording.format.toUpperCase()}</span
                                            >
                            ...
                            {#if renamingRecording !== recording.filename}
                                <div class="recording-actions">
                                    ...
                                    {:else}
                                        <!-- Transcribe Button -->
                                        <button
                                            class="action-btn {isTranscribing[
                                                recording.filename
                                            ]
                                                ? 'loading'
                                                : ''}"
                        ...
                        {#if transcriptionProgress[recording.filename]}
                            <div class="transcription-progress">
                                ...
                                <div class="progress-message">
                                    {transcriptionProgress[recording.filename]
                                        .message}
                                </div>
                            </div>
                        {/if}
...
<style>
    ...
    .skeleton-icon {
        ...
        background: linear-gradient(
            90deg,
            rgba(0, 103, 192, 0.08) 0%,
            rgba(0, 103, 192, 0.12) 100%
        );
        border-radius: 50%;
