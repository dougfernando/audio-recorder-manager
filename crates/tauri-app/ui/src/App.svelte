<script>
    ...
    afterUpdate(() => {
        if (!firstRenderComplete) {
            ...
            console.log(
                "[TIMING] First render complete (afterUpdate):",
                performance.now() - componentStart,
                "ms",
            );
        }
    });
    ...
    async function handleStartRecording() {
        ...
        // Only start recording if we're on the record tab
        if (activeTab !== "record") {
            console.log(
                "[Keyboard] S key pressed but not on record tab, ignoring",
            );
            return;
        }

        try {
            // Pass the same parameters as the RecordingPanel button
    ...
    onMount(async () => {
        ...
        // Add global keyboard shortcuts listener with capture phase
        // This ensures the handler runs before element-specific handlers
...
<main>
    ...
    <aside class="sidebar">
                    ...
                    {#if $recordings.length > 0}
                        {#each $recordings.slice(0, 8) as recording (recording.path)}
                            <button
                                ...
                                on:click={(event) => {
                                    event.currentTarget.blur();
                                    switchTab("recordings", recording);
                                }}
                            >
        ...
        <div class="sidebar-footer">
            <button
                class="footer-btn {activeTab === 'recovery' ? 'active' : ''}"
                on:click={(event) => {
                    event.currentTarget.blur();
                    switchTab("recovery");
                }}
            >
                ...
                >
                    <path
                        d="M8 2a6 6 0 100 12A6 6 0 008 2zm0 1.5a4.5 4.5 0 110 9 4.5 4.5 0 010-9z"
                    />
                    <path d="M8 5v4l3 1.5" />
                </svg>
                Recovery
